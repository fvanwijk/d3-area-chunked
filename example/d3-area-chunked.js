(function (global, factory) {
  ...
  function render() {
    ...
    var defaultPointAttrs = {// read fill and r at render time in case the areaAttrs changed
    ...
    function getChunkNames(useChunkDefOrder) {
      ...
      var prependGap = true; // if using chunk definition order, only prepend area/gap if they aren't in the
      // chunk definition.
      ...
      function splitSegment(segment, chunkNames) {
        ...
        var segments = segment.reduce(function (segments, d, i) {
          ...
          if (dChunkName === chunkName) {
            ...
            if (dPrev) {
              var segmentChunkName = chunkAreaResolver(chunk(dPrev), dChunkName, chunkNames); // if it is supposed to be in this chunk, add it in
            ...
            if (!added && dNext != null) {
              ...
              var nextSegmentChunkName = chunkAreaResolver(dChunkName, chunk(dNext), chunkNames); // if it's supposed to be in the next chunk, create it
    ...
    function evaluateDefinitions(d, i) {
      ...
      var chunks = getChunkNames(); // for each chunk, evaluate the attrs and styles to use for areas and points

      chunks.forEach(function (chunkName) {
        var chunkDef = chunkDefinitions[chunkName] || {};
        var evaluatedChunk = {
          styles: _objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2({}, evaluateAttrsOrStyles(areaStyles)), evaluateAttrsOrStyles((chunkDefinitions[areaChunkName] || {}).styles)), chunkName === gapChunkName ? evaluateAttrsOrStyles(gapStyles) : undefined), evaluateAttrsOrStyles(chunkDef.styles)),
          attrs: _objectSpread2(_objectSpread2(_objectSpread2(_objectSpread2({}, evaluateAttrsOrStyles(areaAttrs)), evaluateAttrsOrStyles((chunkDefinitions[areaChunkName] || {}).attrs)), chunkName === gapChunkName ? evaluateAttrsOrStyles(gapAttrs) : undefined), evaluateAttrsOrStyles(chunkDef.attrs))
        ...
        evaluatedChunk.pointAttrs = Object.assign(basePointAttrs, evaluateAttrsOrStyles(pointAttrs), evaluateAttrsOrStyles((chunkDefinitions[areaChunkName] || {}).pointAttrs), evaluateAttrsOrStyles(chunkDef.pointAttrs)); // ensure `r` is a number (helps to remove 'px' if provided)
    ...
    function renderCircles(initialRender, transition, context, root, points, evaluatedDefinition, className) {
      ...
      var circlesEnter = circles.enter().append('circle'); // apply user-provided attrs, using attributes from current area if not provided
      ...
      if ((!initialRender || initialRender && transitionInitial) && transition) {
        var enterDuration = transitionDuration * 0.15; // delay sizing up the radius until after the area transition
    ...
    function renderClipRects(initialRender, transition, context, root, segments, _ref, _ref2, evaluatedDefinition, path, clipPathId) {
      ...
      var gDebug = root.select('.d3-area-chunked-debug'); // set up debug group
    ...
    function getAreaFunctions(areaData, initialRender, yDomain) {
      ...
      var initialArea; // if the user specifies to extend ends for the undefined area, add points to the area for them.
      ...
      if (extendEnds && areaData.length) {
        ...
        areaData = [[extendEnds[0], processedAreaData[0][1]]].concat(_toConsumableArray(processedAreaData), [[extendEnds[1], processedAreaData[processedAreaData.length - 1][1]]]); // this area function works on the processed data (default .x and .y read the [x,y] format)
      ...
      if (initialRender) {
        ...
        if (transitionInitial) {
          initialArea = d3Shape.area().x(x).y0(y0).y1(y0).curve(curve); // if the user extends ends, we should use the area that works on that data
    ...
    function initializeClipPath(chunkName, root) {
      ...
      var className = "d3-area-chunked-clip-".concat(chunkName);
    ...
    function renderAreas(initialRender, transition, context, root, data, areaIndex) {
      ...
      // `{ results: [[x,y], [[x,y], ...]}`)
      var areaData = accessData(data); // filter to only defined data to plot the areas

      var filteredAreaData = areaData.filter(defined); // determine the extent of the y values
      ...
      var evaluatedDefinitions = evaluateDefinitions(data, areaIndex); // update area functions and data depending on animation and render circumstances

      var areaResults = getAreaFunctions(filteredAreaData, initialRender); // areaData possibly updated if extendEnds is true since we normalize to [x, y] format

      var area = areaResults.area,
          initialArea = areaResults.initialArea,
          modifiedAreaData = areaResults.areaData; // for each chunk type, render a area

      var chunkNames = getChunkNames();
      var definedSegments = computeDefinedSegments(areaData); // for each chunk, draw a area, circles and clip rect

      chunkNames.forEach(function (chunkName) {
        var clipPathId = initializeClipPath(chunkName, root);
        var className = "d3-area-chunked-chunk-".concat(chunkName);

        if (chunkName === areaChunkName) {
          className = "d3-area-chunked-defined ".concat(className);
        } else if (chunkName === gapChunkName) {
          className = "d3-area-chunked-undefined ".concat(className);
